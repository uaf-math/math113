...
\def\s{1}
\begin{center}
\begin{tikzpicture}[vtx/.style={draw, circle, inner sep = 3pt}, myto/.style={-latex, shorten >=2pt, shorten <=2pt
}, node distance = \r cm]
\node[vtx, label=above:$T_{1}(10)$] (T1) at (0,0){};
\node[vtx, right =of T1, label=above:$T_{2}(7)$] (T2) {};
\node[vtx, right =of T2, label=above:$T_{3}(4)$] (T3) {};
\node[vtx, below = \s cm of T1, label=above:$T_{4}(9)$] (T4) {};
\node[vtx, right =of T4, label=above left:$T_{5}(5)$] (T5) {};
\node[vtx, right =of T5, label= right:$T_{6}(4)$] (T6) {};
\node[vtx, below =\s cm of T4, label=below:$T_{7}(13)$] (T7) {};
\node[vtx, right =of T7, label=below:$T_{8}(4)$] (T8) {};
\node[vtx, right =of T8, label=below:$T_{9}(6)$] (T9) {};
...
\item Create a schedule using the same priority list 
\[T_{1}, T_{2}, T_{3}, T_{4}, T_{5}, T_{6}, T_{7}, T_{8}, T_{9}\]
assuming you have three processors.
...
\item Consider the same digraph:
\def\r{2}
\def\s{1}
\begin{center}
\begin{tikzpicture}[vtx/.style={draw, circle, inner sep = 3pt}, myto/.style={-latex, shorten >=2pt, shorten <=2pt
...
The Decreasing Time Algorithm says: Create the priority list by listing the tasks in order from longest completion time to shortest completion time.

\be
\item What priority list do you get if you prioritize the tasks using the Decreasing Time Algorithm?

\hrulefill

\item  Create a schedule using the priority list you just found,
assuming you have only two processors.
