...
\item \emph{Eulerize this graph:} find the smallest number of edges you can duplicate so that you can construct an \emph{Euler circuit}, and add them to the graph.
\item Draw the circuit on the graph (offset slightly so you can see it) so that you can follow it without lifting your pencil from the circuit.
...
\tikzstyle{vtx}=[circle, draw, inner sep=2pt]%, minimum size=6pt]
\foreach \i in {0,1,2}{
\path let \n1 = {int(\i+0)} in node[vtx] (v\n1) at (2*\i,2*0){$V_{\n1}$};
\path let \n1 = {int(\i+3)} in node[vtx] (v\n1) at (2*\i,2*1){$V_{\n1}$};
\path let \n1 = {int(\i+6)} in node[vtx] (v\n1) at (2*\i,2*2){$V_{\n1}$};
 }
 
 \path (v0) node[vtx, ultra thick] {$V_{0}$};
  \path  (v8) node[vtx, ultra thick]{$V_{8}$};


\foreach \i/\j/\k in {0/1/2, 0/3/5, 0/4/8, 1/2/5, 2/5/1, 3/4/6, 4/5/2, 1/4/1, 3/6/7, 3/7/1, 4/7/2, 4/8/9, 5/8/3, 6/7/2, 7/8/1, 1/5/6}{
...
\be
\item There are two vertices of odd degree in this graph, $V_{0}$ and $V_{8}$. Use Dijkstra's algorithm to find a path of minimum (weighted) distance between the two vertices. Break ties by using the vertex whose subscript is smaller (for example, $V_{1}$ comes before $V_{3}$ since $1 < 3$). List the vertices in order you explored them to the right.

\begin{minipage}[t]{.7\linewidth}
\vspace{0pt}
\begin{tabular}{l | c | c | c | c | c | c | c | c| c }
vertex & $V_{0}$ & $V_{1}$ & $V_{2}$ & $V_{3}$ & $V_{4}$ & $V_{5}$ & $V_{6}$ & $V_{7}$ & $V_{8}$ \\ \hline
explored? & &&&&&&&\\ \hline
