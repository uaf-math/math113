...
\lhead{\emph{Math F113X: Polyalphabetic Ciphers}}
...
%\end{center}
\begin{enumerate}
\item \emph{Polyalphabetic Cipher}

\vspace{1in}

\item \emph{Progressive Caesar Cipher / Sequential Shift}: Start with a Caesar cipher, and then shift one letter with each character you are encrypting.
...
\item \emph{Vigen\`ere Cipher} Choose a keyword, and use that keyword to determine a shift cipher for each letter. (Repeat the keyword over and over.)
