...
\begin{enumerate}
	...
	(FYI: Once the \emph{current} vertex is explored, it will be labelled \emph{visited} and never considered again.)
	\item  Let $v$ be the current vertex. For every vertex $w$ with an edge to $v$ \emph{is not marked as \textbf{visited}}, calculate the distance to $e$ through $v.$ If this distance is smaller than the present distance, update $w$ with the new distance.** Otherwise, do nothing. \\
	(FYI: This number is called the tentative distance to $e$.)
	\item Mark the current vertex as \emph{visited.} We never look at this vertex again.
